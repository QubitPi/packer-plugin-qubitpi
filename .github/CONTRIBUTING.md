...
If you have never worked with Go before, you will have to install its runtime in order to build packer with the plugin.
...
Setting up Plugin for Dev
-------------------------

With Go installed, you can already `go get` the plugin and `make dev` in order to compile and test it. These
instructions target POSIX-like environments (macOS, Linux, Cygwin, etc.) so you may need to adjust them for Windows or
other shells.

1. Download the plugin source (and its dependencies) by running `go get github.com/QubitPi/packer-plugin-qubitpi`.
   This will download the source to `$GOPATH/src/github.com/QubitPi/packer-plugin-qubitpi`.
2. When working on the plugin, first `cd $GOPATH/src/github.com/QubitPi/packer-plugin-qubitpi` so you can run `make dev`
   and easily access other files. `make dev` will build the packer-plugin-qubitpi binary and install it under
   `$HOME/.packer.d/plugins/`.
3. Make your changes to the plugin source. You can run `make dev` to build and install locally, and `make test` to run
   unit tests. Any compilation errors will be shown when the binaries are rebuilding. If you don't  have `make` you can
   simply run `go build -o packer-plugin-qubitpi` from the project root, and
   `mv packer-plugin-qubitpi ~/.packer.d/plugins/packer-plugin-qubitpi` to install the plugin.
4. After building the plugin successfully, use the latest version of Packer to build a machine and verify your changes.
   In the [example folder](https://github.com/QubitPi/packer-plugin-qubitpi/blob/main/example) we provide a basic
   template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous
   step.
...
need to [fork the plugin](https://github.com/QubitPi/packer-plugin-qubitpi/fork), push your changes to your fork, and
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-qubitpi f-my-feature
```
...
The easiest way to work on a fork is to set it as a remote of the plugin project.

1. Navigate to the code:

   ```shell
   cd $GOPATH/src/github.com/QubitPi/packer-plugin-qubitpi
   ```
...
2. Add the remote by running:
   ...
   ```shell
   git remote add myuser https://github.com/myuser/packer-plugin-qubitpi.git
   ```
...
Please only apply the minimal vendor changes to get your PR to work. The plugin does not attempt to track the latest
